#!/usr/bin/env python3
# Gerichte boosterronde: alleen de 4 hardnekkige gevallen (idx199,380,723,806) die na de
# eerste DPO-ronde (v2) nog niet waren opgelost. Vertrekt vanaf v2 (niet vanaf origineel),
# meer epochs op minder voorbeelden om het gedrag echt te laten beklijven, zelfde lage lr.
# De 4 al-opgeloste voorbeelden (349,362,529,620) worden hier NIET herhaald, om te voorkomen
# dat extra gradient-updates die al-geleerde correctie verstoren.
...
DATA = "/workspace/train_dpo_v3_booster.jsonl"
OUT = "/workspace/dpo_gemma31b_grounding-adapter_v3"
...
print(f"DPO-paren (booster): {len(rows)} -- idx {[r['idx'] for r in rows]}", flush=True)
...
if MAX_STEPS > 0:
    ...
    print(f"=== BOOSTER: {EPOCHS} epochs op {len(rows)} voorbeelden = {EPOCHS*len(rows)} stappen ===", flush=True)
...
print("DPO-TRAINING_KLAAR_V3", flush=True)
