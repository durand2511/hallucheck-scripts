...
SHORT_RULE = (
    "NARROW-PROGRAM-NAME RULE: if the facts list repeatedly names a specific, narrowly-scoped "
    "program (e.g. an acronym or named plan) as the source of every eligibility rule, and the "
    "question itself does not use that specific program's name, do not assume the rules apply -- "
    "say you cannot confirm eligibility for the general term the question uses, since the "
    "document's rules are all scoped to that narrower named program only.\n\n"
)
COMPOSE_SYSTEM = COMPOSE_SYSTEM_BASE + SHORT_RULE
print(f"=== Prompt-toevoeging: {len(SHORT_RULE)} tekens (was {len(COMPOSE_SYSTEM_BASE)}, nu {len(COMPOSE_SYSTEM)}) ===", flush=True)
...
LABEL = os.environ.get("LABEL", "v2_plus_short_prompt")
...
for i, row in enumerate(rows):
    ...
    import re as _re
    _end = _re.search(r"</self_described>", extraction, flags=_re.IGNORECASE)
    if _end:
